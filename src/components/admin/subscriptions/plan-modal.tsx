...
import { createSubscriptionPlan, updateSubscriptionPlan } from "@/services/admin.service";
...
export function PlanModal({ plan, trigger }: PlanModalProps) {
  ...
  const onSubmit: SubmitHandler<PlanFormValues> = async (values) => {
    ...
    try {
      ...
      if (response?.success) {
        SuccessToast(response.message || `Plan ${plan ? 'updated' : 'created'} successfully!`);
        setIsOpen(false);
        if (!plan) form.reset();
      } else {
  ...
  return (
    <ModalWrapper
      ...
      description={plan ? "Update the details and limitations of this plan." : "Define a new subscription tier with specific features and pricing."}
      showClose={true}
      actionTrigger={
        trigger || (
          <Button variant={plan ? "ghost" : "default"} size={plan ? "icon" : "default"}>
            {plan ? <Pencil className="h-4 w-4" /> : <Plus className="mr-2 h-4 w-4" />}
            {!plan && "Create New Plan"}
          </Button>
        )
      }
    >
      <form onSubmit={form.handleSubmit(onSubmit)} className="p-6 max-h-[80vh] overflow-y-auto">
        <FieldGroup>
          {/* Section 1: Identity */}
          <FieldSet>
            <FieldLegend>Plan Details</FieldLegend>
            <FieldDescription>Identity and identification for this tier.</FieldDescription>
            <FieldGroup className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                <Field data-invalid={!!errors.name}>
                  <FieldLabel htmlFor="p-name">Plan Name</FieldLabel>
                  <Input id="p-name" placeholder="e.g. Pro" {...form.register("name")} />
                  <FieldError errors={[errors.name]} />
                </Field>
                <Field data-invalid={!!errors.code}>
                  <FieldLabel htmlFor="p-code">Unique Code</FieldLabel>
                  <Input id="p-code" placeholder="pro_tier" {...form.register("code")} disabled={!!plan} />
            ...
            <FieldSet>
              ...
              <FieldDescription>Set the cost and member limitations.</FieldDescription>
              <FieldGroup className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                ...
                <Field data-invalid={!!errors.maxMembers}>
                  ...
                  <Input id="p-members" type="number" {...form.register("maxMembers")} />
            ...
            <FieldSet>
              ...
              <FieldDescription>Select features included in this plan.</FieldDescription>
                <FieldGroup className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3 mt-4">
                {Object.keys(FEATURE_LABELS).map((key) => {
                  ...
                  return (
                    <Field key={key} orientation="horizontal">
                      ...
                      />
                      <FieldLabel htmlFor={`f-${key}`} className="font-normal cursor-pointer">
                        {FEATURE_LABELS[key]}
                      </FieldLabel>
                    </Field>
                  );
            ...
            <FieldSet>
              ...
              <FieldDescription>Visible to messes and selection logic.</FieldDescription>
                <FieldGroup className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                <Field orientation="horizontal" className="justify-between rounded-lg border p-4 bg-muted/5">
                  <div className="space-y-0.5">
                    <FieldLabel htmlFor="s-active" className="text-base font-medium py-0">Active Status</FieldLabel>
                    <FieldDescription className="text-xs">Visible to messes.</FieldDescription>
                  </div>
                  <Controller
                    control={form.control}
                    name="isActive"
                ...
                <Field orientation="horizontal" className="justify-between rounded-lg border p-4 bg-muted/5">
                  <div className="space-y-0.5">
                    <FieldLabel htmlFor="s-default" className="text-base font-medium py-0">Default Plan</FieldLabel>
                    <FieldDescription className="text-xs">Pre-selected for new messes.</FieldDescription>
                  </div>
                  <Controller
                    control={form.control}
                    name="isDefault"
